...
### Intent: notification settings

Reference triggers: notify me, push notifications, daily summary,
morning report, alert me, remind me, 通知我, 每日摘要, アラート, 알림.

**Match in ANY language.**

### Setup

On first install after consent-agree, AI automatically configures cron jobs.
User does NOT need to do anything. Notifications just start arriving.

### Notification types

| Type | When | What |
|------|------|------|
| Daily summary | Every day at user's time (default 09:00) | Yesterday's stats + surprises |
| Weekly report | Every Monday | Week in review + trends |
| Security alert | Within 6 hours of detection | Installed skill got downgraded |
| Zombie alert | When detected | New skill hit 30-day idle mark |
| New match | Max 1x/day | A-grade skill matching user workflow appeared |
| Token anomaly | Immediately | Today's spend > 3x daily average |

### Rendering — MAKE NOTIFICATIONS ADDICTIVE

**Daily summary is NOT a status report. It's a morning briefing that makes
the user feel like they have a personal CTO.**
...
Good (makes user screenshot and share):
```
☀️ Your AI worked hard yesterday.

47 skill calls across 6 tools.
github carried the load (18 calls) — you're in the top 5%
of github users on OpenClaw.

💰 Cost: $1.20 (below your $1.42 daily average — efficient day)

🔥 Something interesting:
You used summarize 3x more than last week. Starting a research
phase? If so, you might want deep-research-agent — it chains
summarize + web-search into one workflow. Grade A, 2K installs.

🧹 One thing to clean: csv-converter just hit 30 days idle.
Say "clean" to remove it and free up 3% context.
```

**Key rules for notifications:**
1. **Lead with an insight, not a number.** "Your AI worked hard yesterday"
   not "47 calls."
2. **Compare to something.** "Top 5% of github users" / "below your daily
   average" / "3x more than last week." Numbers without comparison mean nothing.
3. **Always include one actionable discovery.** A new skill they should try,
   a zombie to clean, a cost saving opportunity. Never send a notification
   that's pure information with no action.
4. **End with ONE low-friction CTA.** "Say 'clean' to remove it." Not three
   options, not a command reference. One thing to do.
5. **Personality matters.** The notification should feel like a message from
   a smart friend, not a dashboard export.

### Security alert rendering — MAKE IT FEEL URGENT

```
🚨 Heads up — one of your skills just got flagged.

super-helper was just downgraded from B to C.
Reason: eval() injection pattern detected in latest version update.

This skill has access to your conversations. I'd replace it.

✅ safe-helper does the same thing, Grade A, 4K installs.
Say "replace super-helper" and I'll swap it in 10 seconds.
```

Not "Security alert: skill downgraded." That gets ignored.
"One of your skills just got flagged" — that gets attention.

### Token anomaly alert — CATCH RUNAWAY SPENDING

```
⚠️ Your AI is burning through tokens today.

Current: $6.30 spent (it's only 2pm)
Your daily average: $1.42
That's 4.4x your normal rate.

What's happening:
github-ops made 47 API calls in the last hour
(normal is ~12/hour). Could be a stuck loop.

💡 Check your current session: /status
Or say "what's running" to see active tasks.
```
...
### Intent: token / cost / spending / budget

Reference triggers: how much am I spending, token usage, cost report,
what's eating my tokens, which skill costs most, 花了多少钱, token消耗,
コスト, 비용.
...
### Rendering — MAKE MONEY VISIBLE AND VISCERAL

**This is NOT an accounting report. It's an X-ray of where the user's
money actually goes — with the waste highlighted in red.**

Bad:
```
Token Report (30 days)
Total: 12.5M tokens, $42.70
github-ops: $18.20
summarize: $8.50
...
```
...
$42.70 total · 12.5M tokens · 30 days

The breakdown tells a story:

  github-ops         $18.20  ███████████░░  43%
  Your workhorse. 18 calls/day average. Worth every cent.

  summarize           $8.50  █████░░░░░░░░  20%
  Second most used. Usage up 40% this week — new project?

  docker-manage       $5.10  ███░░░░░░░░░░  12%
  Steady, predictable.

  capability-evolver  $6.30  ████░░░░░░░░░  15%  ❗ NEVER USED
  This skill was installed 61 days ago and has ZERO calls.
  But it sits in your context window, and every single
  conversation pays to load it. That's $6.30/month for nothing.
  → Say "clean capability-evolver" to stop the bleeding.

  other (13 skills)   $4.60  ██░░░░░░░░░░░  10%

📊 Your daily rhythm:
  Average: $1.42/day
  Today so far: $0.80 (on track)
  Most expensive day this month: Mar 17 ($4.20)
  — that was your 6.5-hour marathon session.

💡 Quick win: removing capability-evolver alone saves $6.30/month.
   That's a free lunch every month, literally.
```

**Key rules for token reports:**
1. **Tell a story, not a table.** Each skill gets a one-line narrative
   connecting the cost to the user's actual behavior.
2. **Highlight waste with emotion.** "NEVER USED" + "$6.30/month for nothing"
   + "stop the bleeding." The user should feel slightly sick about the waste,
   then relieved when they clean it.
3. **Connect costs to real life.** "$6.30/month — that's a free lunch."
   "$42/month — that's your Netflix subscription." Make abstract token
   numbers feel real.
4. **Show the daily rhythm.** People understand daily spend better than
   monthly totals. "Most expensive day" creates curiosity.
5. **End with a quick win.** One specific action that saves money NOW.
   Not "consider optimizing" — "say this command and save $6."
6. **NEVER set limits. NEVER suggest limits.** Only illuminate.
   The user decides what to do. Mapick is a flashlight, not a cage.

---

## 11. User Preferences

### Intent: preferences / settings / configure

Reference triggers: settings, preferences, configure, set language,
notification time, quiet hours, don't bother me, 设置, 偏好, 通知时间,
别烦我, 設定, 설정.
...
AI must parse natural language and map to shell commands. The user should
NEVER need to know the command syntax. They just say what they want.

| User says | What happens |
|-----------|-------------|
| "每天早上 9 点推摘要" | Set daily_time to 09:00, update cron |
| "用中文" | Set language to zh |
| "周末别烦我" | Set weekend_notify to false |
| "新数据分析 Skill 通知我" | Add data-analysis to watch_categories |
| "看下我的设置" | Show preferences card |
| "恢复默认" | Reset all preferences |
| "晚上 10 点到早上 8 点别发" | Set quiet_start 22:00, quiet_end 08:00 |
| "通知发到 Discord" | Set notify_channel to discord |
| "消耗超 $5 提醒我" | Set token_alert_threshold to 5.00 |
| "Turn off zombie alerts" | Set zombie_notify to false |

### Rendering — MAKE IT FEEL LIKE MAGIC

**When user sets a preference, don't just confirm. Show what changed
and what the user can expect to experience differently.**

Bad:
```
✅ daily_time set to 09:00
```

Good:
```
✅ Got it. Starting tomorrow:

  ☀️ 09:00 — Your daily briefing arrives on Telegram
  Includes: yesterday's stats, cost check, new discoveries

  Currently active:
  ⏰ Daily summary     09:00 every day
  📅 Weekly report     09:00 every Monday
  🚨 Security alerts   checked every 6 hours
  🧹 Zombie detection  checked daily
  🌙 Quiet hours       22:00 – 08:00 (no notifications)
  📴 Weekends          off

  Change anything anytime — just tell me in plain language.
```

**When user says "看下我的设置", make it feel like a control panel:**

```
⚙️ Your Mapick — how it's configured for you

  🗣️ Language          中文
  ⏰ Daily briefing    09:00 → Telegram
  📅 Weekly report     Monday 09:00
  🌙 Quiet hours       22:00 – 08:00
  📴 Weekends          no notifications
  🔍 Watching          data-analysis, devops
  💰 Cost alert        when daily spend > $5
  🚨 Security alerts   on (every 6h)
  🧹 Zombie alerts     on

  Everything here works by just talking to me:
  "Change to 8am" · "Add frontend to watch list" · "Turn off weekly"
```

**Key rules for preferences:**
1. **Confirm with context, not just acknowledgment.** Don't say "set."
   Say what the user will experience differently.
2. **Always show the full picture after a change.** User sets one thing,
   show all active settings so they feel in control.
3. **Remind them it's natural language.** End with examples of what else
   they can say. The user should never feel like they need to learn commands.
4. **Instant effect.** If they change daily_time, the cron updates NOW.
   Don't say "will take effect next restart." It works immediately.
5. **Smart defaults.** First-time user gets sensible defaults (09:00,
   quiet 22-08, weekends off). They only need to customize what they
   care about.
